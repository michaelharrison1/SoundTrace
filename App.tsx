...
const AppContent: React.FC = () => {
  ...
  const handleLogout = () => {
    try {
      localStorage.removeItem('authToken');
      localStorage.removeItem('currentUserDetails');
      if (spotifyPlayerHook && spotifyPlayerHook.disconnectSpotify) {
        spotifyPlayerHook.disconnectSpotify().catch(err => {
          console.error("Error during Spotify disconnect on logout:", err);
          // Even if Spotify disconnect fails, proceed with app logout
        });
      }
    } catch (error) {
      console.error("Error clearing auth data or calling disconnectSpotify:", error);
    }
