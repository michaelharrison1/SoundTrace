...
const AuthHeaderContent: React.FC<AuthHeaderContentProps> = ({ currentUser, authView, onSetAuthView, onLogout }) => {
  const { disconnectSpotify: spotifyDisconnectHook } = useSpotifyPlayer(); // Get the hook

  const handleFullLogout = async () => {
    try {
      await spotifyDisconnectHook(); // Call Spotify disconnect first
    } catch (error) {
      console.error("Error during Spotify disconnect on logout (non-critical):", error);
    }
  ...
  if (currentUser) {
    ...
    return (
      <>
        ...
        <Button
          onClick={() => onSetAuthView('register')}
          size="sm"
